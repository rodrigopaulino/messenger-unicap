	...
	public void run() {
		while (true) {
			...
			try {
				...
				if (acaoRequisitada.equals(Constantes.ID_ACAO_LOGIN)) {
				...
				} else if (acaoRequisitada.equals(Constantes.ID_ACAO_LOGOUT)) {
					enderecoRemetente = leitorEntrada.readLine();
					socket.close();
					...
					try {
						...
						System.out.println(
							"Logout pode não ter sido efetuado com sucesso. Alguns usuarios ainda podem estar vendo o usuario deslogado!");
					}
				}
			} catch (IOException e) {
